...
def show_bottleneck_representation_demo(baseline_weights_file=None, manifold_mixup_weights_file=None):
    ...
    visualizer.show_b2_model_hidden_representation(
        mnist_model_b2, x_train, y_train)
    visualizer.show_b2_model_hidden_representation(
        mnist_model_b2_with_mixup, x_train, y_train)
...
def show_spiral_demo(baseline_model_weights_file=None, mixup_model_weights_file=None):
    ...
    visualizer.plot_spiral_model_confidence(
        spiral_model, x_train, y_train, title='no mixup')
    visualizer.plot_spiral_model_confidence(
        spiral_model_with_mixup, x_train, y_train, title='manifold mixup')


def main():
    show_bottleneck_representation_demo(
        baseline_weights_file='./{}/{}'.format(
            TRAINED_MODELS_DIR, 'bottleneck_2_no_mixup_1588680905625058000.h5'),
        manifold_mixup_weights_file='./{}/{}'.format(
            TRAINED_MODELS_DIR, 'bottleneck_2_manifold_mixup_1588704453481012000.h5')
    )

    show_svd_demo(
        baseline_weights_file='./{}/{}'.format(
            TRAINED_MODELS_DIR, 'bottleneck_12_no_mixup_1588699264167078000.h5'),
        input_mixup_weights_file='./{}/{}'.format(
            TRAINED_MODELS_DIR, 'bottleneck_12_input_mixup_1588699435781023000.h5'),
        manifold_mixup_weights_file='./{}/{}'.format(
            TRAINED_MODELS_DIR, 'bottleneck_12_manifold_mixup_1588702304001298000.h5'),
    )

    show_spiral_demo(
        baseline_model_weights_file='./{}/{}'.format(
            TRAINED_MODELS_DIR, 'spiral_no_mixup_1588698972067361000.h5'),
        mixup_model_weights_file='./{}/{}'.format(
            TRAINED_MODELS_DIR, 'spiral_manifold_mixup_1588682681909873000.h5'),
    )
