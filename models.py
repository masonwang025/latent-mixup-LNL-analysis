...
class ManifoldMixupModel(tf.keras.models.Sequential):
    def __init__(self, layers_tuple, mixup_alpha=None, name=None):
        super(ManifoldMixupModel, self).__init__(
            [layer for layer, _ in layers_tuple], name=name)
        self.alpha = mixup_alpha
        self.eligible_mixup_layers = [
            layer for layer, eligible_for_mixup in layers_tuple if eligible_for_mixup]
    ...
    def call(self, inputs, training=False, mixup_mode=False, y_true=None):
        ...
        for layer in self.layers:
            if can_mixup and layer == k:
                layer_activation, y_true = self.manifold_mixup(
                    layer_activation, y_true)
    ...
    def train_step(self, x, y_true):
        with backprop.GradientTape() as tape:
            y_pred, y_mixed_true = self(
                x, training=True, mixup_mode=True, y_true=y_true)
    ...
    return ManifoldMixupModel(
        [
            ...
            (tf.keras.layers.Dense(12, activation=tf.keras.layers.LeakyReLU(
                alpha=LEAKY_RELU_ALPHA), name='bottleneck'), include_manifold_mixup),
            (tf.keras.layers.Dense(1024, activation=tf.keras.layers.LeakyReLU(
                alpha=LEAKY_RELU_ALPHA)), False),
            (tf.keras.layers.Dense(1024, activation=tf.keras.layers.LeakyReLU(
                alpha=LEAKY_RELU_ALPHA)), False),
...
def create_spiral_model(mixup_mode=MixupMode.NO_MIXUP):
    ...
    return ManifoldMixupModel(
        [
            (tf.keras.layers.Dense(1024, activation=tf.keras.layers.LeakyReLU(
                alpha=LEAKY_RELU_ALPHA), input_shape=(2,)), include_input_mixup),
            (tf.keras.layers.Dense(1024, activation=tf.keras.layers.LeakyReLU(
                alpha=LEAKY_RELU_ALPHA)), False),
            (tf.keras.layers.Dense(1024, activation=tf.keras.layers.LeakyReLU(
                alpha=LEAKY_RELU_ALPHA)), False),
